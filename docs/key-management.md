...
backup YubiHSMs and one out of two backup USB thumb drives function at all times.
...
Secret passphrases are stored redundantly on `n = 2` USB thumb drives.  Use
...
`authkey` and `wrapkey` are stored on 2x USB thumb drives as described above.
